...
/// quantity, optionally a customer name — `unitPrice` is deliberately
/// omitted from the request so backend-api snapshots the SKU's current
/// price itself, exactly like every other order-creation caller.
  ...
  Future<void> _submit() async {
    ...
    try {
      await ref.read(ordersServiceProvider).createOrder(
        lines: [CreateOrderLine(skuId: sku.id, quantity: quantity)],
      ...
      setState(() {
        _isSubmitting = false;
        _error = 'Không thể tạo đơn hàng. Có thể sản phẩm đã hết hàng.';
      });
    }
  }
